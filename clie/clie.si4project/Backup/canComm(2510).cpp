...
unsigned short filter_id[12]={
	      /*per 10ms*/	0x00F9+1,	
		  /*per 25ms*/	0x0175+1,0x0189+1,0x0197+1,0x019D+1,0x01A6+1,0x01AF+1,0x01F5+1,
		  /*per 50ms*/	0x02A5+1,
		  /*per 250ms*/	0x03F5+1,
		  /*per 500ms*/	0x04A5+1,0x04C9+1
		  					};
...
void CanComm::canFilter(int& s,bool isSet = false)
{
    if(!isSet)
        return;
	for(unsigned int i=0;i<sizeof(filter_id);i++)
		{
