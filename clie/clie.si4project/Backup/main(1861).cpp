...
//		printf("task%d's param past is :%d \n",i,sched_param[i].sched_priority);
    ...
    int ret1 = pthread_create(&th[0],&attr[0],Task::WaitingForServer,(void*)&cliMsg);//等待服务器数据

    int ret2 = pthread_create(&th[1],&attr[1],Task::WaitingForCan,(void*)&canMsg);//接收can报文

    int ret3 = pthread_create(&th[2],&attr[2],Task::MonitorSendBuffer,(void*)&cliMsg);//监视接收eth的buf

	int ret4 = pthread_create(&th[3],&attr[3],Task::MonitorRecvBuffer,(void*)&cliMsg);//监视接收can的buf
