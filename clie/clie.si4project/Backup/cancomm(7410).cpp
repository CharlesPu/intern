...
bool CanComm::CanInit(int channel)
{
	...
	switch (channel)
		{
			case 1:	strcpy(_ifr1.ifr_name,"can0");//指定can设备
					can_channel=0;
					break;
			case 2:	strcpy(_ifr1.ifr_name,"can1");//指定can设备
					can_channel=1;
					break;
			case 3:	strcpy(_ifr1.ifr_name,"can0");//指定can设备
					can_channel=2;
					break;
			case 4:	strcpy(_ifr1.ifr_name,"can1");//指定can设备
					can_channel=3;
					break;	
...
{	
	struct can_filter m_filter[recvid_on_bus_size[0]+recvid_on_bus_size[1]+recvid_on_bus_size[2]+recvid_on_bus_size[3]
								- recvid_on_bus_size[can_channel]	];//can的接收过滤器是由另外三个can总线的recvid数组决定的
	int size=0;
	for(int i=0;i<4;i++)
		{
