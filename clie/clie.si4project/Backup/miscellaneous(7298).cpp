...
//			}
		  /**************拆分成can报文：id+data+校验和 ***************/
			canfr.can_id=((client->c_rxBuf[ret-3]&0xffff)<<8|client->c_rxBuf[ret-2])+1;
			//printf("can_id=%d~\r\n",canfr.can_id);
			canfr.can_dlc=ret-3;	
			//printf("datalength=%d~\r\n",canfr.can_dlc);	
			char xorsum=0;
...
void* WaitingForCan(void* ctp)
{
	...
	Buffer* _cbuffer = ctp_tmp->_sbuffer;
    ...
    while(1)
    {
        ...
        //for(i = 0; i < master; i++)
        {
			...
			/**************封装成以太网帧：data+id+校验和 ***************/				
				...
				ret=_sbuffer->PutFrame(_client->c_txBuf , &(_sbuffer->recv_q),
						fr.can_dlc+3);
			/**************目前can接收缓冲区的内容 ***************/
...
void* MonitorBuffer(void* cltp)
{
	...
	Buffer* sbuffer = cltp_temp->_pctp->_sbuffer;	
