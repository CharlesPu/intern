    ...
    while(1)
    {
		...
		/**************封装成以太网帧：报文总长+id+data+异或校验和 ***************/
			...
			}else              			//如果要发给以太网
			{
				...
				int r=_serv_buf->PutFrame(temp, &(_serv_buf->send_q),temp[0]);
#ifdef PRINT_PROCESS
					...
					while(serv_temp->m_rxBuf[frame_head] != 0)
						{
							...
							if(xorsum == serv_temp->m_rxBuf[frame_head + frame_len -1])
								...
								if(IsBelongTo(id_temp,(void*)can_eth_para_temp->_can))//如果属于第一个can的
									...
									int r=_can_buf->PutFrame(&(serv_temp->m_rxBuf[frame_head]), 
										&(_can_buf->send_q), frame_len);
#ifdef PRINT_PROCESS
									printf("another can's frame:");//将从客户端读到的数据，在屏幕上输出
									for (int i = 0; i < r; i++)
										printf("0x%02x ",serv_temp->m_rxBuf[frame_head+i]);
									printf("\n");
#endif	
									...
									{
									pthread_mutex_lock(&(can_eth_para_temp->_another_can->send_lock));
									int r=another_can_buf->PutFrame(&(serv_temp->m_rxBuf[frame_head]), 
