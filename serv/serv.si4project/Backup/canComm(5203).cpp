...
bool CanComm::socketInit(int& s,struct ifreq& ifr)
{
    ...
    strcpy(ifr.ifr_name,"can1");//指定can1设备
    ...
    ret = bind(s, (struct sockaddr *)&addr, sizeof(addr));//将套接字与can1绑定
    if (ret < 0)
    {
        perror("bind failed");
        return false;
...
void CanComm::canFilter(int& s,bool isSet = false)
{
    ...
    m_filter[0].can_id = 0x001 ;
    m_filter[0].can_mask = CAN_SFF_MASK;
    /* 第2个规则是可以接收ID为0x025 的数据帧 */
    //m_filter[1].can_id = 0x025 ;
    //m_filter[1].can_mask = CAN_SFF_MASK;
    /* 第2个规则是可以接收ID为0x1F5 & 0xFFF的数据帧 */
    //m_filter[2].can_id = 0x1F5 | CAN_SFF_MASK;
