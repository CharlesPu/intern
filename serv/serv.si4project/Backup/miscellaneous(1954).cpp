...
//				}
			  ...
			  /**************目前can发送缓冲区的内容 ***************/
						...
						{
						   for(int j=0;j<ret;j++)
						   		printf("sendbuf[%d]=%d",j,sbuffer->send_q.q_buf[k][j]);
						   printf("\n");
						}							
					}
				else
    ...
    while(1)
    {
        ...
        //for(i = 0; i < master; i++)
        {
			...
			/**************封装成以太网帧：data+id+校验和 ***************/				
				...
				ret=_sbuffer->PutFrame(_server->m_txBuf, &(_sbuffer->recv_q),fr.can_dlc+3);
		    /**************目前can接收缓冲区的内容 ***************/
				...
				{
				   for(int j=0;j<ret;j++)
				   		printf("recvbuf[%d]=%d",j,_sbuffer->recv_q.q_buf[i][j]);
				   printf("\n");
				}
			}	   	   
            //usleep(100);
	...
	while(1)
		...
		if(!EMPTY(sbuffer->recv_q))
			{	
				...
				sbuffer->GetFrame(sbuffer->recv_q, s_to_c_temp,  );
				ret=server->SocketSend(server->_ClientSock,s_to_c_temp,    ); 		
			/**************最终发出去的帧，调试用 ***************/	
//				for(int i=0;i<   ;i++)
//				{
...
//				}
		...
		if(!EMPTY(sbuffer->send_q))
			{	
				...
				sbuffer->GetFrame(sbuffer->send_q, c_to_s_temp);
			/**************拆分成can报文：id+data***************/	
