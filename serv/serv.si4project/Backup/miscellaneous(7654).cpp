...
void* WaitingForCan(void* ctp)
{
    ...
    char 	t_buff[20] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
    ...
    while(1)
    {
        ...
        //for(i = 0; i < master; i++)
        {
			...
			if(fr.can_dlc>0)
            {
				...
				vecStr.clear();
				*/				
                t_buff[0]=(fr.can_id>>8)&0xff;
				t_buff[1]=fr.can_id&0xff;
				for(i=2;i<2+fr.can_dlc;i++)
				{
                   t_buff[i]=fr.data[i-2];
				}
				for(i=0;i<2+fr.can_dlc;i++)
				{
				   ret=_server->SocketSend(_server->_ClientSock,&t_buff[i]);
				}
				printf("length=%d\r\n",2+fr.can_dlc);
//			    if(ret=_server->m_tx_cryptLen)
//				{
//				    printf("id1=%d\r\n",t_buff[0]);
//					printf("id2=%d\r\n",t_buff[1]);
//					printf("data=%d\r\n",t_buff[2]);
//			    	printf("both send out by server~\r\n");
//				}	
