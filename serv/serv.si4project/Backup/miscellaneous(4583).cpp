...
//				}
			  ...
			  /**************拆分成can报文：id+data+校验和 ***************/
				canfr.can_id=((server->m_rxBuf[ret-3]&0xffff)<<8|server->m_rxBuf[ret-2])+1;
				//printf("can_id=%d~\r\n",canfr.can_id);
				canfr.can_dlc=ret-3;	
				//printf("datalength=%d~\r\n",canfr.can_dlc);	
				char xorsum=0;
				for(int j=0;j<2+canfr.can_dlc;j++)
					{
    ...
    while(1)
    {
        ...
        //for(i = 0; i < master; i++)
        {
			...
			/**************封装成以太网帧：data+id+校验和 ***************/				
				...
				printf("ready\n");
				ret=_buffer->PutFrame(_server->m_txBuf, _buffer->recv_q,fr.can_dlc+3);
				printf("ret=%d",ret);
//				for(i=0;i<CHARS(_buffer->recv_q);i++)
//				{
//				   printf("_buffer->recv_qbuf[%d]=",i);
//				   puts(_buffer->recv_q.q_buf[i]);
//				   printf("\n");
//				}
...
//				}
				...
				//ret=_server->SocketSend(_server->_ClientSock,_server->m_txBuf,fr.can_dlc+3);	
                printf("***************************************************\r\n");
			}	   	   
            //usleep(100);
        }
